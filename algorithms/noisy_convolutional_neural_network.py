...
class NoisyConvolutionalNeuralNetwork(nn.Module):
    def __init__(self, algorithm_name, env_name, network_type, in_dim: int, out_dim: int):
        """Initialization."""
        super(NoisyConvolutionalNeuralNetwork, self).__init__()
        ...
        self.noisy_layer1 = NoisyLinear(64 * 7 * 7, 512)
        self.noisy_layer2 = NoisyLinear(512, out_dim)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward method implementation."""
        feature = self.conv(x)
        feature = feature.view(-1, 64 * 7 * 7)
