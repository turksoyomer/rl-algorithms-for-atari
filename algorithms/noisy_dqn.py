...
from .noisy_neural_network import NoisyNeuralNetwork
from .noisy_convolutional_neural_network import NoisyConvolutionalNeuralNetwork
import pickle
import os

class NoisyDQNAgent:
    """Noisy DQN Agent interacting with environment.
    ...
    def __init__(
        ...
        if use_conv:
            self.behavior_network = NoisyConvolutionalNeuralNetwork(self.algorithm_name, self.env_name, 'behavior', obs_dim[0], action_dim).to(self.device)
            self.target_network = NoisyConvolutionalNeuralNetwork(self.algorithm_name, self.env_name, 'target', obs_dim[0], action_dim).to(self.device)
        else:
            self.behavior_network = NoisyNeuralNetwork(self.algorithm_name, self.env_name, 'behavior', obs_dim[0], action_dim).to(self.device)
            self.target_network = NoisyNeuralNetwork(self.algorithm_name, self.env_name, 'target', obs_dim[0], action_dim).to(self.device)
    ...
    def select_action(self, state: np.ndarray) -> np.ndarray:
        ...
        selected_action = self.behavior_network(torch.FloatTensor(state).to(self.device)).argmax()
